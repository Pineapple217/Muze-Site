...
def signup(request):
    if not request.user.is_authenticated:
        if request.method == "POST":
            ...
            if user_form.is_valid() and lid_form.is_valid():
                ...
                send_mail(
                    ...
                    render_to_string(
                        "leden/mail/succes_signup.html", {"name": user.first_name}
                    ),
        ...
        return render(
            request, "leden/signup.html", {"user_form": user_form, "lid_form": lid_form}
        )
    else:
...
def edit_profile(request):
    ...
    return render(
        request,
        "leden/edit_profile.html",
        {"forms": (user_form, lid_form), "pp_size": config.PP_MAX_SIZE_MB},
    )
        ...
        for user_id, opt in post_data.items():
            ...
            if opt == "accept":
                ...
                send_mail(
                    _("Welkom to The Muze"),
                    render_to_string(
                        "leden/mail/accepted.html", {"name": user.first_name}
                    ),
                    settings.EMAIL_HOST_USER,
                    [user.email],
                    fail_silently=False,
        ...
        if password_reset_form.is_valid():
            ...
            if associated_users.exists():
                for user in associated_users:
                    ...
                    email_template_name = (
                        "registration/password/password_reset_email.html"
                    )
                    mail_context = {
                        "email": user.email,
