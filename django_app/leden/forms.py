...
class LidSignUpForm(ModelForm):
    tel = forms.RegexField(
        regex=r"^\+[0-9]{2,3}\ [0-9]{3}(\ [0-9]{2}){3}$",
        label=_l("Phone number"),
        help_text=_l("Format example +32 989 91 23 12."),
    )

    date_of_birth = forms.DateField(
        widget=forms.DateInput(attrs={"type": "date"}), label=_l("Date of birth")
    )
...
class LidUpdateForm(ModelForm):
    ...
    class Meta:
        ...
        help_texts = {
            "profile_picture": _l("Image size must be under %(max)s MB.")
            % {"max": config.PP_MAX_SIZE_MB},
        }
